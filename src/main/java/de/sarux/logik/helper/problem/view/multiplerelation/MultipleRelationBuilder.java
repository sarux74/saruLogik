package de.sarux.logik.helper.problem.view.multiplerelation;
...
public class MultipleRelationBuilder {
    ...
    public MultipleRelationBuilder(GeneralLogikProblem currentProblem) {
        this.problem = currentProblem;
    }

    public List<GeneralLogikBlockViewLine> build() {
        List<GeneralLogikBlockViewLine> result = new ArrayList<>();
    ...
    private void findCrossRelations(List<GeneralLogikBlockViewLine> result) {
        final Map<LogikGroup, List<LogikLineRelation>> relationsPerGroup = new HashMap<>();
        for (final GeneralLogikBlock block : problem.getSingleBlocks()) {
    ...
    private void findNextBeforeRelations(List<GeneralLogikBlockViewLine> result) {
        final Map<LogikGroup, List<LogikLineRelation>> relationsPerGroup = new HashMap<>();
        for (final GeneralLogikBlock block : problem.getSingleBlocks()) {
            for (final LogikLineRelation relation : block.getRelations()) {
    ...
    private void buildMultipleRelationBlock(LogikGroup group, LogikLine key, Collection<LogikLine> lowerLines, Collection<LogikLine> higherLines, List<GeneralLogikBlockViewLine> result, String relationHint) {
        String name = "Gruppe " + group.getName() + " - Zeile " + key.getLineId();
        GeneralLogikBlockViewLine blockLine = new GeneralLogikBlockViewLine(ViewLineType.BLOCK, 0, null, null, null);
        ...
        for (LogikLine showLine : lowerLines) {
            result.add(GeneralLogikBlockViewBuilder.buildLineViewLine(problem.getGroups(), 0, 0, showLine));
        }

        GeneralLogikBlockViewLine rightViewLine = new GeneralLogikBlockViewLine(ViewLineType.RELATION_UPPER, null, 0, key.getLineId(), 0);
        ...
        result.add(GeneralLogikBlockViewBuilder.buildLineViewLine(problem.getGroups(), 0, 0, key));
        ...
        result.add(new GeneralLogikBlockViewLine(ViewLineType.MAIN_SEPARATOR, 0, 0, null, null));
    }

    private void buildMultipleRelationBlock(LogikLineRelation logikLineRelation, LogikGroup group, List<GeneralLogikBlockViewLine> result) {
        ...
        GeneralLogikBlockViewLine blockLine = new GeneralLogikBlockViewLine(ViewLineType.BLOCK, 0, 0, null, null);
        ValueView valueView = new ValueView(name);
        blockLine.setValueView(0, valueView);
        result.add(blockLine);
        ...
        GeneralLogikBlockViewLine rightViewLine = new GeneralLogikBlockViewLine(ViewLineType.RELATION_UPPER, 0, 0, showLine.getLineId(), 0);
        String text = "* = ";
        ValueView relationValueView = new ValueView(text);
        ...
        rightViewLine = new GeneralLogikBlockViewLine(ViewLineType.RELATION_LOWER, 0, 0, lowerLine.getLineId(), 0);
        text = "*";
        relationValueView = new ValueView(text);
        rightViewLine.setValueView(rightGroup.getIndex(), relationValueView);

        result.add(rightViewLine);
        result.add(GeneralLogikBlockViewBuilder.buildLineViewLine(problem.getGroups(), 0, 0, lowerLine));
        result.add(new GeneralLogikBlockViewLine(ViewLineType.MAIN_SEPARATOR, 0, 0, null, null));
    }
