...
export class GroupComponent implements OnInit {
  ...
  startSolver() {
    if(this.problemType === 0) {
      this.groupService.initProblem().subscribe(result => { console.log('Problem initialisiert!');
        this.problemType = 1;
        this.openSolver();
      });
    } else {
  ...
  startDetektor() {
    if(this.problemType === 0) {
        this.groupService.initDetektor().subscribe(result => { console.log('Detektor initialisiert!');
          this.problemType = 2;
          this.openDetektor();
        });
      } else {
